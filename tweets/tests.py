...
class TweetTestCase(TestCase):
    def setUp(self):
        self.user = User.objects.create_user(username='michel', password='anewnewpassword')
        self.user2 = User.objects.create_user(username='michel2', password='anewnewpassword2')
        Tweet.objects.create(content="my first tweets", user=self.user)
        Tweet.objects.create(content="my second tweets", user=self.user)
        Tweet.objects.create(content="my third tweets", user=self.user2)
    ...
    def get_client(self):
        client = APIClient()
        client.login(username=self.user.username, password='anewnewpassword')
        return client
